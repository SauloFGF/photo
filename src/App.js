...
const App = () => {
    ...
    function filterapp() {
        if (atr === 'blur') {
            return { filter: `${atr}(${brig}px)` }
        }
        else if (atr === "hue-rotate") {
            return { filter: `${atr}(${brig}deg)` }
        } else {
            return { filter: `${atr}(${brig}%)` }
        }
    }
                ...
                <div className="gallery-container">
                    {imagens.map(function (image) {
                        return <img onClick={() => {
                            setImage(image)
                            setBlur({ value: "0" })
                        }} className='img-gallery' src={image} alt='' />
                    })}
                </div>
                <div className="grad">
                    <img className="viewer" src={image} alt='' style={filterapp()} />
